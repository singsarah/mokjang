...
### 4.3 일정 `/calendar`
- 자체 CSS grid 월간 뷰(외부 라이브러리 없음), **일요일 칸 3배 폭**. 셀 탭 → 팝업, 상단 "이번 달 목록".
...
2. 요약 카드: 세션 출석 n/재적 + ◀▶ 세션 이동 + 출석/사유결석/미확인 3칸(탭하면 명단) + 반별 한 줄
3. 출석 추이 그래프: **이번주 포함 직전 4주 고정**(주=일~토, 주에 마감 세션 여러 개면 마지막 것), 출석/결석 토글(결석=사유결석+미확인 두 색 스택), 모든 막대 위 인원수·y축 눈금, 자료 없는 주는 빈 칸 + **출석부 다운로드**
